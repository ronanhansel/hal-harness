...
class AutoInspector:
    ...
    def __init__(
        ...
        self.system_prompt = (
            "You are the HAL AutoInspector. Given rubric evidence and a trace tail, examine the HAL agent "
            "source code and produce a precise diagnostic for a coding agent to act on.\n\n"
            "Hard constraint:\n"
            "- The coding agent must NOT run the HAL debugger or any rerun commands.\n"
            "- Your guidance must be sufficient to produce a fix package in one pass, with careful self-review.\n\n"
            ...
            "- When the rubric indicates an environmental barrier (missing executables, permissions, missing deps, "
            "read-only paths, etc.), your job is to propose a fix package that makes the run succeed in this "
            "evaluation environment.\n"
            "- Prefer environment-scoped solutions compatible with the harness:\n"
            ...
            "- If the current prompt injection instructs the agent to avoid remediation (e.g. "
